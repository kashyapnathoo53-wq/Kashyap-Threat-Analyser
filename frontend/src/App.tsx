...
export const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen cyber-bg text-slate-100 font-sans flex flex-col selection:bg-cyan-500 selection:text-slate-950 relative">
      {/* Topmost Enterprise Status Bar */}
      <div className="bg-slate-950/80 border-b border-white/[0.05] px-4 py-1 text-[11px] font-mono flex items-center justify-between text-slate-400">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1.5 text-emerald-400 font-bold">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping inline-block" />
            SOC TELEMETRY: REAL-TIME ACTIVE
          </span>
          <span className="hidden sm:inline text-slate-600">|</span>
          <span className="hidden sm:inline text-slate-400">HOST TARGET: <strong className="text-slate-200">{hostAssessment?.host_info?.hostname || 'LOCAL_ENDPOINT'}</strong></span>
          <span className="hidden md:inline text-slate-600">|</span>
          <span className="hidden md:inline text-slate-400">OS: <span className="text-cyan-300">{hostAssessment?.host_info?.os || 'Windows NT 64-bit'}</span></span>
        </div>

        <div className="flex items-center gap-3">
          <span className="px-2 py-0.5 rounded bg-cyan-950/80 text-cyan-300 border border-cyan-800/60 text-[10px] font-bold">
        ...
        <div className="max-w-7xl mx-auto px-4 py-3.5 flex justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3.5">
            <div className="relative group cursor-pointer">
              <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-cyan-500 via-sky-400 to-indigo-600 p-[1.5px] shadow-lg shadow-cyan-500/25 transition-transform duration-300 group-hover:scale-105">
                <div className="w-full h-full bg-[#050b18] rounded-2xl flex items-center justify-center">
                  <Shield className="w-6 h-6 text-cyan-400" />
                </div>
              </div>
              <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-cyan-400 border-2 border-slate-950" />
            </div>

            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-lg font-black tracking-tight text-white flex items-center gap-2">
                  KASHYAP <span className="gradient-text-cyan font-extrabold">THREAT ANALYSER</span>
                </h1>
                <span className="text-[10px] font-mono font-extrabold px-2 py-0.5 bg-gradient-to-r from-cyan-500/20 to-indigo-500/20 text-cyan-300 border border-cyan-500/30 rounded-md">
                  UNICORN EDITION
                </span>
              </div>
              <p className="text-xs text-slate-400 font-medium">
                Next-Gen Static, Behavioral &amp; Endpoint Auto-Assessment Cyber Grid
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            {hostAssessment && (
              <button 
                ...
                className="hidden lg:flex items-center gap-2.5 bg-slate-900/90 hover:bg-slate-800/90 px-3.5 py-2 rounded-xl border border-white/[0.08] hover:border-cyan-500/50 transition shadow-inner group"
              >
                <MonitorCheck className="w-4 h-4 text-cyan-400 group-hover:animate-bounce" />
                <div className="text-left text-xs">
                  <div className="text-[10px] text-slate-400 leading-none">Endpoint Posture</div>
            ...
            {report && (
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <div className="font-bold text-cyan-300 max-w-[130px] truncate leading-tight" title={report.sample_name}>
                    {report.sample_name}
                  </div>
                </div>
                {(report as any).analysis_duration_ms && (
                  <span className="text-[10px] text-slate-400 border-l border-slate-700 pl-2 flex items-center gap-1">
        ...
        {loading ? (
          <div className="glass-panel rounded-3xl p-14 flex flex-col items-center justify-center space-y-6 shadow-2xl relative overflow-hidden my-12 border border-cyan-500/30">
            ...
            <div className="relative">
              <div className="w-24 h-24 rounded-full border-4 border-cyan-500/20 border-t-cyan-400 animate-spin flex items-center justify-center shadow-xl shadow-cyan-500/30" />
              <Radio className="w-10 h-10 text-cyan-300 absolute inset-0 m-auto animate-ping opacity-75" />
            </div>

            <div className="text-center space-y-3 z-10 max-w-lg w-full">
              <div className="text-xl font-black tracking-wide text-white">
                Zero-Lag Deep Threat Engine Executing...
              </div>
              <div className="text-xs font-mono text-cyan-300 bg-slate-950/80 py-2 px-4 rounded-xl border border-slate-800">
                {loadingPhase}
              </div>

              {/* Futuristic Progress Bar */}
              <div className="w-full bg-slate-950 rounded-full h-3 border border-slate-800 overflow-hidden mt-3 shadow-inner">
                <div 
          ...
          <>
            ...
            <div className="flex overflow-x-auto glass-panel p-1.5 rounded-2xl border border-white/[0.08] text-xs font-bold gap-1.5 shadow-2xl">
              {[
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all duration-200 whitespace-nowrap text-xs font-bold ${
                      isActive 
                        ? 'bg-gradient-to-r from-cyan-500 to-sky-500 text-slate-950 shadow-lg shadow-cyan-500/30 scale-[1.02]' 
      ...
      <footer className="glass-panel border-t border-white/[0.06] mt-auto py-5 px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-3">
          <div className="flex items-center gap-3">
            ...
            <span>KASHYAP THREAT ANALYSER &bull; ENTERPRISE CYBER INTELLIGENCE</span>
          </div>
