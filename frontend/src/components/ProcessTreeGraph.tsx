      ...
      <div 
        className={`flex items-center gap-2 p-2.5 rounded-lg border transition-all ${
          isMaliciousProc 
            ...
            : 'bg-slate-900/60 border-lime-900/30 text-slate-200 hover:border-lime-500/50'
        }`}
      >
        {hasChildren ? (
          <button 
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className={`font-mono font-bold text-sm ${isMaliciousProc ? 'text-rose-300' : 'text-lime-300'}`}>{node.name}</span>
