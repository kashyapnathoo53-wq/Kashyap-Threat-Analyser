...
export const SampleSelectorModal: React.FC<Props> = ({ onSelectPreset, onFileUpload, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-fadeIn">
      ...
      <div className="bg-[#120409]/95 border border-rose-600/40 w-full max-w-2xl rounded-3xl p-6 sm:p-7 shadow-2xl shadow-rose-950/60 space-y-6 relative overflow-hidden">
        {/* Mehroon Top Glow Line */}
        ...
        <div className="flex justify-between items-center border-b border-rose-900/30 pb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-rose-950/80 border border-rose-500/40 flex items-center justify-center text-rose-400 shadow-md">
          ...
          <button 
            type="button"
            onClick={onClose} 
            className="text-slate-400 hover:text-white text-xl font-bold w-8 h-8 rounded-xl hover:bg-rose-950/60 flex items-center justify-center transition cursor-pointer border border-transparent hover:border-rose-800"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-2xl p-7 text-center transition-all duration-200 flex flex-col items-center justify-center relative cursor-pointer select-none ${
            isDragActive 
              ? 'border-rose-400 bg-rose-950/40 scale-[1.01] shadow-xl shadow-rose-950/40' 
              : 'border-rose-800/60 hover:border-rose-500 bg-[#1a060e]/80 hover:bg-[#200812]'
          }`}
        >
          <div className="w-14 h-14 rounded-2xl bg-rose-950/80 border border-rose-500/40 flex items-center justify-center text-rose-300 mb-3 shadow-inner">
          ...
          <div className="mt-4 flex items-center gap-3">
            <button
              ...
              className="px-6 py-2.5 bg-gradient-to-r from-rose-700 via-rose-600 to-red-600 hover:from-rose-600 hover:to-red-500 text-white font-black text-xs rounded-xl transition shadow-lg shadow-rose-900/40 cursor-pointer"
            >
        ...
        <div>
          <div className="flex items-center justify-between text-xs font-bold text-rose-300/70 uppercase tracking-wider mb-2.5">
            <span>Or Choose Benchmark Malware Family</span>
            <span className="text-[10px] text-rose-400 font-mono">Instant Emulation</span>
          </div>
            ...
            {loading ? (
              ...
              presets.map(p => (
                <div
                  ...
                  className="bg-[#18050e]/90 p-3 rounded-xl border border-rose-900/40 hover:border-rose-500/60 hover:bg-[#200813] cursor-pointer transition flex justify-between items-center group shadow-sm"
                >
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-bold text-rose-200 text-xs group-hover:text-white transition">{p.name}</span>
                      <span className="px-2 py-0.5 text-[9px] uppercase font-mono font-bold rounded bg-rose-950 text-rose-300 border border-rose-800/60">
                        {p.type}
                      </span>
                    </div>
                  ...
                  <button 
                    type="button"
                    className="px-3.5 py-1.5 bg-rose-950/80 group-hover:bg-rose-600 text-rose-200 group-hover:text-white text-xs font-bold rounded-lg transition flex items-center gap-1 shrink-0 ml-2 border border-rose-900/60 group-hover:border-rose-500 shadow-sm"
                  >
