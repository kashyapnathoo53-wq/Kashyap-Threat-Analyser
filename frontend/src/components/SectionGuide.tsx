    ...
    <div className="glass-card rounded-2xl overflow-hidden border border-white/[0.08] shadow-2xl transition-all duration-300 mb-6 group">
      ...
      >
        <div className="flex items-center gap-3.5">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-orange-600/25 to-amber-700/35 border border-orange-500/40 flex items-center justify-center text-orange-400 shrink-0 shadow-inner">
            <BookOpen className="w-4 h-4" />
          </div>
          <div>
            <div className="flex items-center gap-2.5">
              <h4 className="text-sm font-extrabold text-white tracking-wide">{title}</h4>
              <span className="px-2.5 py-0.5 text-[10px] uppercase font-mono font-black rounded-full bg-orange-950/90 text-orange-300 border border-orange-500/40 shadow-sm">
                {badge}
              </span>
            </div>
        ...
        <button 
          ...
          className="text-xs font-mono font-bold text-orange-300 hover:text-white flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900/90 border border-orange-500/30 hover:border-orange-400 hover:bg-orange-950/60 transition shadow-sm cursor-pointer"
        >
      ...
      {isExpanded && (
        <div className="px-5 pb-5 pt-2 border-t border-white/[0.06] space-y-4 text-xs animate-fadeIn bg-slate-950/40">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* What it does */}
            <div className="glass-card p-4 rounded-xl border border-orange-900/40 bg-slate-900/60">
              <div className="flex items-center gap-2 text-orange-400 font-black mb-2 text-xs uppercase tracking-wider">
                <Shield className="w-4 h-4 text-orange-400" /> What Does This Section Do?
              </div>
              ...
              {keyIndicators.map((ind, idx) => (
                <div key={idx} className="bg-slate-950/80 p-3 rounded-xl border border-white/[0.05] text-[11px] shadow-sm">
                  <span className={`font-mono font-black block mb-1 ${
                    ...
                    ind.severity === 'high' ? 'text-amber-400' : 'text-orange-300'
                  }`}>
                    {ind.label}
                  </span>
