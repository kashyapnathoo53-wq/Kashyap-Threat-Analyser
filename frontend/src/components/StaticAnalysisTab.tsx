          ...
          {[
            ...
            >
              ...
              <button
                ...
                className="p-2 bg-slate-900 hover:bg-cyan-400 hover:text-slate-950 text-slate-400 rounded-xl transition shrink-0 border border-white/[0.06]"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 rounded-3xl">
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={entropyData} margin={{ top: 10, right: 20, left: -20, bottom: 0 }}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#040d1e', borderColor: '#06b6d4', borderRadius: '12px', fontSize: '12px' }}
                  formatter={(val: any) => [`${val} / 8.0`, 'Entropy']}
                />
                <Area type="monotone" dataKey="entropy" stroke="#00f2fe" strokeWidth={2} fillOpacity={1} fill="url(#entropyGradient)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="mt-4 pt-3 border-t border-white/[0.06] text-xs text-slate-400 flex items-center justify-between font-mono">
            ...
            <span>Packer: <span className="text-sky-300">{staticAnalysis.pe_structure.packer || "None Detected"}</span></span>
          </div>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            ...
            <div className="flex rounded-xl bg-slate-950 p-1 border border-white/[0.08] text-xs font-bold font-mono">
              <button
                ...
                className={`px-3 py-1 rounded-lg transition ${stringTab === 'decoded' ? 'bg-gradient-to-r from-cyan-500 via-sky-500 to-blue-600 text-slate-950 font-black shadow-md shadow-cyan-950/40' : 'text-slate-400 hover:text-white'}`}
              >
                Decoded ({staticAnalysis.strings.decoded.length})
              </button>
              <button
                onClick={() => setStringTab('ascii')}
                className={`px-3 py-1 rounded-lg transition ${stringTab === 'ascii' ? 'bg-gradient-to-r from-cyan-500 via-sky-500 to-blue-600 text-slate-950 font-black shadow-md shadow-cyan-950/40' : 'text-slate-400 hover:text-white'}`}
              >
                ASCII ({staticAnalysis.strings.ascii_total})
              </button>
              <button
                onClick={() => setStringTab('unicode')}
                className={`px-3 py-1 rounded-lg transition ${stringTab === 'unicode' ? 'bg-gradient-to-r from-cyan-500 via-sky-500 to-blue-600 text-slate-950 font-black shadow-md shadow-cyan-950/40' : 'text-slate-400 hover:text-white'}`}
              >
              ...
              {filteredDecoded.length > 0 ? (
                filteredDecoded.map((s, idx) => (
                  <div key={idx} className="p-2.5 bg-slate-900/80 rounded-xl border border-white/[0.05] flex items-center justify-between gap-3">
                    <div className="truncate">
                      <span className="text-[10px] uppercase font-bold text-cyan-400 block">{s.type}</span>
            ...
            <div className="space-y-1">
              {filteredUnicode.length > 0 ? (
                filteredUnicode.map((s, idx) => (
                  <div key={idx} className="py-1 px-2 hover:bg-slate-900 rounded text-cyan-300 select-all truncate border-b border-slate-900/50">
                    {s}
                  </div>
                ))
              ) : (
