    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 rounded-3xl">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-extrabold text-white flex items-center gap-2">
              <FileCode className="w-4 h-4 text-rose-400" /> Cryptographic File Fingerprints
            </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 font-mono text-xs">
          {[
            ...
            <div 
              key={i} 
              className="bg-slate-950/80 p-3.5 rounded-2xl border border-white/[0.05] flex items-center justify-between gap-3 group hover:border-rose-500/40 transition shadow-sm"
            >
              <div className="truncate">
                <span className="text-[10px] text-slate-500 uppercase tracking-wider block font-sans font-bold mb-0.5">
                  {h.label}
                </span>
                <span className="text-slate-300 text-xs truncate block select-all group-hover:text-rose-300 transition">
                  {h.val}
                </span>
              </div>
              <button
                onClick={() => copyToClipboard(h.val, h.label)}
                className="p-2 bg-slate-900 hover:bg-rose-600 hover:text-white text-slate-400 rounded-xl transition shrink-0 border border-white/[0.06]"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 rounded-3xl">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-extrabold text-white flex items-center gap-2">
                <Cpu className="w-4 h-4 text-rose-400" /> Section Shannon Entropy Curve
              </h3>
              <p className="text-xs text-slate-400">
          ...
          <div className="mt-4 pt-3 border-t border-white/[0.06] text-xs text-slate-400 flex items-center justify-between font-mono">
            <span>Overall File Entropy: <strong className="text-rose-300">{staticAnalysis.file_info.entropy}</strong></span>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="px-2.5 py-1 text-[10px] font-mono font-bold rounded-lg bg-rose-950/80 text-rose-300 border border-rose-800">
                {staticAnalysis.suspicious_imports.length} APIs Flagged
              </span>
            </div>

            <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
              {staticAnalysis.suspicious_imports.map((item, idx) => (
      ...
      <div className="glass-card p-6 rounded-3xl">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
          <div>
            <h3 className="text-sm font-extrabold text-white flex items-center gap-2">
              <Binary className="w-4 h-4 text-rose-400" /> Extracted Strings &amp; Auto-Decoded Streams
            </h3>
            <p className="text-xs text-slate-400">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            ...
            <div className="flex rounded-xl bg-slate-950 p-1 border border-white/[0.08] text-xs font-bold font-mono">
              <button
                ...
                className={`px-3 py-1 rounded-lg transition ${stringTab === 'decoded' ? 'bg-rose-600 text-white font-black shadow-md shadow-rose-950/40' : 'text-slate-400 hover:text-white'}`}
              >
                Decoded ({staticAnalysis.strings.decoded.length})
              </button>
              <button
                onClick={() => setStringTab('ascii')}
                className={`px-3 py-1 rounded-lg transition ${stringTab === 'ascii' ? 'bg-rose-600 text-white font-black shadow-md shadow-rose-950/40' : 'text-slate-400 hover:text-white'}`}
              >
                ASCII ({staticAnalysis.strings.ascii_total})
              </button>
              <button
                onClick={() => setStringTab('unicode')}
                className={`px-3 py-1 rounded-lg transition ${stringTab === 'unicode' ? 'bg-rose-600 text-white font-black shadow-md shadow-rose-950/40' : 'text-slate-400 hover:text-white'}`}
              >
                ...
                filteredDecoded.map((s, idx) => (
                  <div key={idx} className="p-2.5 bg-slate-900/80 rounded-xl border border-white/[0.05] flex items-center justify-between gap-3">
                    <div className="truncate">
                      ...
                      <span className="text-rose-300 font-bold text-xs select-all">{s.decoded}</span>
                    </div>
            ...
            <div className="space-y-1">
              {filteredUnicode.length > 0 ? (
                filteredUnicode.map((s, idx) => (
                  <div key={idx} className="py-1 px-2 hover:bg-slate-900 rounded text-rose-300 select-all truncate border-b border-slate-900/50">
                    {s}
                  </div>
                ))
              ) : (
